/*
 * RF KM Switch — console PTX (Step 4a: forked from NCS esb_ptx, parity build).
 *
 * Runs on the Seeed XIAO nRF52840 Plus inside the desk console. Today this is
 * the upstream sample verbatim apart from log/Kconfig renames — transmits a
 * dummy 8-byte payload and cycles LEDs on ack. Step 4b swaps the payload for
 * the typed KB/mouse format defined in docs/rf_protocol.md; Step 4d adds the
 * UART link from the RP2040.
 *
...
int main(void)
{
	...
	LOG_INF("RF KM Switch console PTX (parity build)");
