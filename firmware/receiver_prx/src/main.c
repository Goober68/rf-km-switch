/*
 * RF KM Switch — receiver PRX (Step 4a: forked from NCS esb_prx, parity build).
 *
 * Runs on a Nordic PCA10059 nRF52840-Dongle plugged into each target host.
 * Today this is the upstream sample verbatim apart from log/Kconfig renames —
 * listens on the default ESB pipe set and cycles LEDs on receive. Step 4b
 * decodes the typed payload (docs/rf_protocol.md); Step 4c hard-pipes the
 * receiver to one machine via CONFIG_KM_RX_PIPE; Step 4e injects HID reports
 * into the host over USB.
 *
...
int main(void)
{
	...
	LOG_INF("RF KM Switch receiver PRX (parity build)");
